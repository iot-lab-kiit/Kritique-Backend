...
import {getFaculty, getFacultyById, findFaculty, updateFaculty, deleteFaculty, deleteAll, createFaculty, viewFaculty} from "../controllers/faculty";
...
router.post("/", async (req, res) => {
    try {
        ...
        if (!name || !experience || !photoUrl) {
            return res.status(400).json({message: "Name, experience, and photoUrl are required"});
        }
